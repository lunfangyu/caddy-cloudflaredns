...
# 配置全局端口
{
        ...
        log {
                ...
                format console {
                        time_format rfc3339 # 日志时间格式：2006-01-02T15:04:05Z07:00
                        time_local
                }
                ...
                output file /var/log/caddy/global.log {
                        ...
                        roll_keep_for 168h # 是将滚动的文件作为[持续时间字符串]保留多长时间。目前的实现支持日的分辨率；小数的值被四舍五入到下一个整日。>例如，36h（1.5天）被四舍五入为168h（7天）。默认值: 2160h (90天)
                }
        }
}
...
example.com:888 *.example.com:888 {
        ...
        # 创建一个名为@server-nas的匹配器 他的匹配规则是请求标头的host字段（即访问的域名）为 nas.example.com
        @server-nas host nas.example.com
        # 处理符合@server-nas匹配器规则的访问 提供反向代理
        handle @server-nas {
